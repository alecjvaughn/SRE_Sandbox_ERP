...
## Overview
This track defines the Infrastructure as Code (IaC) foundation for the SRE Sandbox project. The goal is to allow deploying the entire project from a clean slate using Terraform and ArgoCD, focusing exclusively on a local development environment (Minikube).
...
- **Clean Slate Redeployment**: The configuration must be capable of rebuilding the entire infrastructure and application stack from an empty Minikube cluster.

## Non-Functional Requirements
- **Scalability**: While the target is Minikube, the "App of Apps" ArgoCD structure should be designed so that it can easily be adapted to a cloud environment in the future.
...
- [ ] Terraform successfully bootstraps ArgoCD into the local Minikube cluster.
